...
export async function updateProductInDB(id: string, updates: Partial<Product>): Promise<void> {
  try {
    ...
    const setClause = fields.map(field => {
      ...
      return `${dbField} = ${updates[field as keyof Product]}`;
    }).join(', ');
...
export async function addCategoryToDB(name: string): Promise<string[]> {
  try {
...
export async function deleteCategoryFromDB(name: string): Promise<string[]> {
  try {
